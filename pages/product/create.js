...
export default function create() {
  ...
  const handleDiscountChange = (value) => {
    ...
    const discountedPrice = Math.round(
      productData.price * (1 - discountPercentage / 100)
    );
    setProductData((prev) => ({
      ...prev,
      discountPercentage,
  ...
  const handleImageUpload = (e) => {
    ...
    setProductData((prev) => ({
      ...prev,
      files: [...prev.files, ...files], // Store raw files
      images: [...prev.images, ...newImages], // Store previews
    }));
  
    console.log("After Image Upload:", productData);
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      productData.files.forEach((file) => formData.append("file", file)); // Change to "file"
      formData.append("name", validatedData.name);
      formData.append("description", validatedData.description);
      formData.append("price", validatedData.price);
      formData.append("discountPercentage", validatedData.discountPercentage);
      formData.append("stock", validatedData.stock);
      formData.append("productInfo",JSON.stringify(validatedData.productInfo));

      // Here you would typically send the data to your backend
      console.log(productData);
      const respond = await fetch("/api/product/create", {
        method: "POST",
        body: formData,
      });
  ...
  const ProductPreview = () => (
    <Card className="bg-gray-800 border-gray-700">
      <CardContent className="p-6">
        <div className="aspect-w-1 aspect-h-1 w-full overflow-hidden rounded-lg bg-gray-700 mb-4">
                      ...
                      <TooltipTrigger asChild>
                        <div className="relative">
                          <Input
                            ...
                            placeholder="0"
                          />
                          <HelpCircle className="absolute right-2 top-1/2 transform -translate-y-1/2 text-gray-400 h-5 w-5" />
                        </div>
                      </TooltipTrigger>
